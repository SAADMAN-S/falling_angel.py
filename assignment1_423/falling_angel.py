...
def draw_pause_button():
    ...
    glColor3f(1, 0.75, 0)
    if pause_flag==False:
        midpoint(245, 495, 245, 465)
        midpoint(255, 495, 255, 465)
    else:
        midpoint(245, 495, 245, 465)
        midpoint(245, 495, 270, 480)
        midpoint(245, 465, 270, 480)
def draw_exit_button():
    glColor3f(1, 0, 0)
    midpoint(465, 495, 495, 465)
    midpoint(465, 465, 495, 495)

def draw_restart_button():
    glColor3f(0,0.75,1)
    midpoint(10,485,40,485)
    midpoint(10,485,20,495)
    midpoint(10,485,20,475)
...
def fire_bullet():
    global speed,bullet,bullet_position,shoot,pause_flag,bullet_wait
    if pause_flag==False:
        #if bullet_flag==True:
            if shoot==True:
                bullet += speed*10
...
def draw_plate(x):
    global game_over,loaded
    if game_over == True:
        glColor3f(1, 0, 0)
    elif game_over == False and loaded==False:
        glColor3f(1, 1, 1)
    elif game_over == False and loaded == True:
        glColor3f(0, 0, 1)
...
def keyboardListener(key, x, y):
    global step,bullet_flag,bullet_position,game_over,pause_flag,pl_start,pl_end,shoot,bullet_wait,loaded
    if key == b' ':
        if bullet_flag==True:
            if game_over == False:
                if pause_flag == False:
...
def green_triangle():
    ...
    while i<=25:
        ...
        j=j+1
def move_triangle():
    global triangle_tip,pause_flag
    if pause_flag==False:
...
def red_circle():
    ...
    while j <= 10:
        ...
        j += 0.1
def move_red():
    global red_centre,red_flag,speed,pause_flag
    if pause_flag==False:
        if red_flag==True:
            red_centre[1]-=speed
...
def move_balls():
    global ball_array,array,last_update_time,pl_start,pl_end,position,ball_count,pause_flag
...
def showScreen():
    global loaded,done,end_flag,droplet_flag,droplet_array,game_over,light_coordinates,light,bullet_position,bullet_wait,bullet_flag,pl_start,position,pl_end,array,speed,ball_array,score,click_flag,temp,click_counts,red_flag,red_count,red_centre,green_flag,green_count,triangle_tip,ball_count,shoot,bullet,pause_flag
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
    glLoadIdentity()
    iterate()
    glClearColor(0, 0, 0, 0)
    if game_over==False:
        if math.floor(red_centre[1]-10)<=10 and (pl_start-8<=math.ceil(red_centre[0])<=pl_end+8) and red_centre[2]==0:
            print("Red Centre X:", red_centre[0])
            print("Plate Start X:", pl_start)
            print("Plate End X:", pl_end)
            game_over=True
        if math.floor(red_centre[1]-10)<=10 and (pl_start-8<=math.ceil(red_centre[0])<=pl_end+8) and red_centre[2]==1:
            print("Red Centre X:", red_centre[0])
            print("Plate Start X:", pl_start)
            print("Plate End X:", pl_end)
            bullet_wait=5
        ...
        for i in range(len(array)):
            if array[i] != None:
                brick_x = array[i][0]
                brick_y = array[i][1]
                if (brick_x - 50 <= bullet_position <= brick_x + 5 and
                        brick_y - 5 <= bullet <= brick_y + 15):
                    bullet = 15
                    if shoot==True:
                        light = True
                    shoot = False
                    if bullet_wait>0:
                        bullet_wait-= 1
                    score+=10
                    light_coordinates=[brick_x+10,brick_y]
                    array[i]=None
        ...
        for i in range(len(array)):
            if array[i]!=None:
                draw_brick(array[i][0], array[i][1],array[i][3])
        if len(array)>0:
            if array[len(array) - 1] !=None:
            ...
            if array[i] is not None:
                ...
                for j in range(len(ball_array)):
                    if ball_array[j]!=None:
                        ...
                        if (brick_x - 50 <= ball_x <= brick_x + 5 and
                                brick_y - 5 <= ball_y <= brick_y + 15):
                            array[i]=None
...
def mouse(button, state, x, y):
    global array, temp, click_flag, score,click_counts,pause_flag,exit_flag
            ...
            for i in range(len(array)):
                if array[i] != None:
                    ...
                    if (array[i][0] - 50) <= x <= (array[i][0] + 20) and (array[i][1] - 50) <= 500-y <= (array[i][1] + 50):
                        ...
                        break
        if pause_flag == False:
            if ((10 <= x <= 20) and (5 <= y <= 15)) or ((10 <= x <= 20) and (15 <= y <= 25)) or (
                    (10 <= y <= 20) and (10 <= x <= 40)):
                restart_game()
                print("Game restarted")
        if pause_flag == False:
            if (245 <= x <= 255) and (5 <= y <= 35):
                pause_flag = True
        elif pause_flag == True:
            if (245 <= x <= 270) and ((0.6 * x) - 142 <= y <= (-0.6 * x) + 182):
                pause_flag = False
        if (465 <= x <= 495) and (5 <= y <= 35):
            exit_flag = True
def restart_game():
    global loaded,done,exit_flag,step,pause_flag,time_step,size,end_flag,droplet_flag,droplet_array,game_over,light_coordinates,light,bullet_position,bullet_wait,bullet_flag,pl_start,position,pl_end,array,speed,ball_array,score,click_flag,temp,click_counts,red_flag,red_count,red_centre,green_flag,green_count,triangle_tip,ball_count,shoot,bullet
    array = []
    speed = 0.9
    ball_array = []
    size = 2
